...
|------|----------------------------|
| Manual `docker compose` before `.env` exists (compose `${HOST_SSH_AUTH_SOCK:-...}` default) | `$XDG_RUNTIME_DIR/${SUPER_PROJECTS_NAME}-ssh-agent.sock` |
| After `initializeCommand` when the managed socket is selected | `$HOME/.cache/${SUPER_PROJECTS_NAME}-ssh-agent.sock` |

`ensure-host-ssh-agent` uses `~/.cache/...` on WSLg because the runtime-dir path can be occupied by a **directory**, which prevents `ssh-agent` from binding a socket. Compose keeps the `$XDG_RUNTIME_DIR` default so manual `docker compose down`, `ps`, and `config` work before initialize has run. Those commands must use `--project-directory .devcontainer` so generated `.env` is loaded.
